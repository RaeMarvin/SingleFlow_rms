...
const CompletedTasksThisWeek: React.FC = () => {
  ...
  const completedThisWeekTasks = tasks.filter(task => {
    ...
    // If task has completedAt, use that date
    if (task.completedAt) {
      const completedDate = new Date(task.completedAt);
      return completedDate >= weekStart && completedDate <= weekEnd;
    }
    
    // Fallback: if no completedAt but task is completed, check if created this week
    // This is for backward compatibility with older completed tasks
  ...
  const groupedTasks = groupTasksByDay(completedThisWeekTasks);
            ...
            <div key={dayKey} className="space-y-2">
              ...
              <div className="space-y-2 pl-2">
                {groupedTasks[dayKey].map(task => (
                  <div 
                    key={task.id}
                    className="flex items-start space-x-3 p-3 bg-neutral-50 dark:bg-gray-700 rounded-lg border border-neutral-200 dark:border-gray-600"
                  >
