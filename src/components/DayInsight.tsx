...
const DayInsight: React.FC<DayInsightProps> = ({ todayPercent }) => {
  ...
  const accent = variant === 'low' ? 'text-noise-700' : variant === 'mid' ? 'text-signal-700' : 'text-primary-700';
    ...
    <div className="bg-white rounded-xl shadow-sm p-4 border border-gray-100">
      <div className="flex items-start space-x-3">
        <div className={`flex-shrink-0 mt-1 ${accent}`}>
          {/* Decorative icon */}
          {variant === 'high' ? (
            <svg className="w-8 h-8" viewBox="0 0 24 24" fill="none" stroke="currentColor"><path strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" d="M12 2l2.09 4.26L18.5 7l-3.5 2.5L15.18 14 12 11.8 8.82 14l.18-4.5L5.5 7l4.41-.74L12 2z"/></svg>
          ) : variant === 'mid' ? (
